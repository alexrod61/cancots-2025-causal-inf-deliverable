...
This section of the final deliverable presents a concise thirteen-week lesson plan for a fourth-year undergraduate course in causal inference. It is intentionally **modular rather than prescriptive**: the roadmap is the common core, whereas the number of specialized estimation methods can be adjusted to the students' preparation and the local curriculum. The lesson plan remains limited in the following ways:
...
# How to Read the Content and Resource Mappings
    
The references linked throughout this page form an **instructor-facing resource bank**, not a cumulative list of readings that students are expected to complete (see @tbl-how-to-read).
    
| Label | Meaning |
|:---:|:---:|
| **Core / required content** | Concepts and activities intended for the standard pathway. This label applies to the **content**, not to every reference listed beside it. Instructors should normally select **one principal source or a short excerpt** for student-facing preparation rather than assign every source mapped to a learning goal. |
| **Recommended resource** | Complementary readings, alternative explanations, published examples, or instructor-preparation materials that can deepen a core topic without being required for all students. **Unless an individual reference is explicitly marked Core, this is the default status of references listed in a core week.** |
| **Optional/advanced** | Specialized technical content and its associated resources that can be taught selectively when student preparation and course time permit. |

: Interpretation of content and resource labels used throughout the course materials. {#tbl-how-to-read .striped .hover}

At the module level, **weeks 1–5 and 8–9 are the core instructional pathway**. Weeks 6 and 7 (G-estimation and causal mediation analysis) are optional/advanced modules.
...
3. **Classify** different types of research questions in terms of causation and association (**understand** level).
4. **Identify** the components of a causal question, e.g., treatment, outcome, and population (**remember** level).
...
2. **Core published-paper activity.** The instructor could provide short excerpts or a structured summary of the following four published papers (two of which pose causal questions, whereas the other two pose association questions). Students should not be expected to read all four papers in full. In addition to classifying each research question, students can begin reconstructing the roadmap by identifying which elements are stated explicitly and which remain implicit in the article:
  - **Causal questions:**
...
2. Identify and distinguish **confounders, mediators, and colliders**, and use open/closed paths and the back-door criterion to explain how conditioning decisions can remove or introduce bias (**analyze** level).
...
1. To address this goal, students could gain hands-on practice by constructing DAGs for cases drawn from **Chapter 1 (Introduction)** of @morgan2007, particularly **sections 1.3.1 (Broad Examples from Sociology, Economics, and Political Science)** and **1.3.2 (Narrow and Specific Examples)**, which provide examples across several applied settings.
...
**Week 3** focuses on both the theoretical and practical aspects of **counterfactuals**. As outlined in the [course roadmap](03-course-roadmap.qmd#sec-course-roadmap), the course now makes an explicit bridge between two causal languages that students may encounter in different disciplines. **DAGs/structural causal models (SCMs)** are used to encode structural causal assumptions, whereas **potential-outcome notation** is used to define counterfactual quantities and treatment-effect estimands. The two perspectives can inform one another, but they should not be introduced to beginners as if they were identical frameworks. This week therefore emphasizes how the graphical/structural assumptions from week 2 connect to, rather than replace, potential-outcome reasoning.
...
1. [Chapters 3 (Observational studies) and 5 (Interaction)](https://static1.squarespace.com/static/675db8b0dd37046447128f5f/t/6835bdb7dc9e482163d8712d/1748352541538/hernanrobins_WhatIf_27may25.pdf) from @hernan2024. These chapters offer comprehensive theoretical coverage. For in-class material, [Roadmap Step 1 - Causal Model (Lecture 2a - Intro to Structural Causal Models)](https://drive.google.com/file/d/117tfMFkQ6OS30cA13cJVus21kfywSh1U/view), from @petersen2014, is a fair starting point.
2. **Chapter 2 (The Counterfactual Model)** from @morgan2007; this chapter offers a comprehensive theoretical content. For in-class material, [Roadmap Step 1 - Causal Model (Lecture 2b - How SCMs encode causal assumptions)](https://drive.google.com/file/d/117tfMFkQ6OS30cA13cJVus21kfywSh1U/view), from @petersen2014, is a fair starting point.
...
1. [Chapters 3 (Observational studies) and 5 (Interaction)](https://static1.squarespace.com/static/675db8b0dd37046447128f5f/t/6835bdb7dc9e482163d8712d/1748352541538/hernanrobins_WhatIf_27may25.pdf) from @hernan2024. These chapters offer comprehensive theoretical coverage.
...
## Defining the Observed-Data and Statistical Problem: Identifiability Conditions (stage 4 in the roadmap)

During **week 4**, the instructor is expected to connect the causal model represented by DAGs and SCMs with the observed-data and statistical problem, with a focus on the identifiability of causal parameters. We start by recalling DAGs as formal representations of causal assumptions. From these DAGs, we construct SCMs, specifying structural equations and endogenous variables that determine counterfactual outcomes. Once the SCM is defined, we analyze how these counterfactuals relate to observed data, emphasizing that only a subset of potential outcomes can be observed.

A key focus this week is on the spectrum of assumptions in statistical modelling. **Parametric** models specify a complete probability distribution (for example, ordinary least-squares regression with normally distributed errors), while **semi-parametric** models relax some of these specifications (like Cox models). **Non-parametric** statistical models avoid imposing a fixed finite-dimensional functional form, although causal identification still requires substantive assumptions (such as consistency, exchangeability, and positivity) that are conceptually distinct from the statistical model itself.
...
2. Assess the plausibility of key identifiability conditions—including consistency, exchangeability/no unmeasured confounding, and positivity—in different study designs (**evaluate** level).
3. Compare parametric, semi-parametric, and non-parametric statistical models, explaining how their assumptions affect estimation and inference while distinguishing them from the causal assumptions required for identification (**understand** level).
...
## Propensity Scores and Inverse Probability Weighting (stages 4 and 5 in the roadmap)

**Week 5** introduces **propensity scores and inverse probability weighting (IPW)** as the first core methods in the course for estimating causal effects from observational data. For a binary treatment, the propensity score $e(L)=P(A=1\mid L)$ provides a particularly useful bridge from students' prerequisite knowledge of binary logistic regression to causal estimation [@rosenbaum1983]. Students first model the treatment-assignment mechanism, inspect overlap and covariate balance, and then translate the estimated propensity scores into weights. The same propensity-score idea can also motivate matching or subclassification as recommended extensions, without requiring a separate full module.

Building on the identifiability assumptions discussed in week 4, we will now focus on the statistical estimation of causal parameters using **weights** derived from the treatment assignment mechanism. Students will be guided through both the conceptual rationale and the mathematical foundations of IPW. Specifically, we will explore how IPW reweights observations to approximate a **pseudo-population in which measured baseline confounders are balanced across treatment groups**. Under assumptions such as consistency, exchangeability/no unmeasured confounding, and positivity, together with appropriate estimation of the treatment mechanism, weighted contrasts can be used to estimate the target causal effect. Students will also examine diagnostics for overlap, covariate balance, and extreme weights.
...
1. Define the **propensity score** for a binary treatment and explain how it connects the treatment-assignment mechanism to weighting or matching strategies (**remember** and **understand** levels).
2. Explain how IPW uses propensity scores to reweight observations and balance measured baseline confounders across treatment groups (**understand** level).
3. Examine the assumptions required for valid IPW estimation, including exchangeability/no unmeasured confounding, positivity, consistency, and adequate treatment-model specification, and discuss consequences of violations (**analyze** level).
4. Distinguish propensity-score weighting from matching/subclassification, recognizing these as different uses of the same balancing-score idea (**understand** level).

The above goals can be mapped to the following materials. For a standard offering, the first two items below are **core**, while the remaining sources serve as recommended depth or extensions:

1. **Core:** @rosenbaum1983 provide the foundational definition of the propensity score and explicitly discuss matching and adjustment based on the score. [Chapter 14 (Matching)](https://theeffectbook.net/ch-Matching.html) from @huntington2021 provides a more accessible bridge to propensity-score estimation and matching for students with regression background.
2. **Core:** [Roadmap Step 6B - Estimation with IPW](https://drive.google.com/file/d/1X1CMuTbRT77tDS9X1RtXSrKRy6Bf02NC/view) from @petersen2014 provides a concise route from the identified estimand to the IPW estimator, including intuition and positivity diagnostics.
3. **Recommended:** [Chapter 12 (IP Weighting and Marginal Structural Models)](https://miguelhernan.org/whatifbook) from @hernan2024 provides a more technical treatment of IPW and marginal structural models.
4. **Recommended applied bridge:** @heiss2020 demonstrates how a binary-treatment propensity model can be fitted with familiar regression tools and translated into weights using both `{ipw}` and `{WeightIt}`. Matching/subclassification can be discussed here as alternative uses of propensity scores without requiring a separate required module.
...
2. @ipw provide a simulated example via the package `{ipw}`.
...
## Optional/Advanced Module: G-estimation for Causal Parameters (stages 4 and 5 in the roadmap)

::: {.callout-important}
**Status: optional/advanced.** This module is retained because it provides a valuable introduction to longitudinal causal inference and time-varying confounding, but it is **not required in the core thirteen-week pathway**. If omitted, these two sessions can be used to deepen propensity-score/IPW diagnostics and matching, framework translation, or published-paper analysis.
:::
...
3. Conduct model diagnostics for G-estimation models (**apply** level). 

The above goals can be mapped to the following materials:

1. @loh2023 provide a tutorial on G-estimation using simulated data via the `{lavaan}` package. In contrast, @DTRreg offer a tutorial using the `{DTRreg}` package, employing both totally simulated data and simulated data inspired by real data.
...
## Optional/Advanced Module: Causal Mediation Analysis (stages 4 and 5 in the roadmap)

::: {.callout-important}
**Status: optional/advanced.** This module is retained because mechanism questions arise frequently in applied research, but the identification assumptions and estimands are conceptually demanding. It is **not required in the core thirteen-week pathway**. If omitted, these sessions can instead deepen core propensity-score/IPW work, matching, framework translation, or published-paper appraisal.
:::

Causal mediation analysis (CMA) allows researchers to go beyond merely estimating total causal effects and to examine the **underlying mechanisms** through which these effects occur. In **week 7**, students will learn how to decompose causal effects into **direct effects** (the part of the treatment effect not explained by a mediator) and **indirect effects** (the portion transmitted through a mediator). By studying mediation analysis within the course framework, students will gain insights into the challenges of making causal claims about underlying mechanisms. This week will also introduce counterfactual definitions of natural direct and indirect effects. Additionally, students will explore how to implement these concepts in practice by linking theoretical assumptions to the modelling framework used in the `{mediation}` R package [@mediation].

![Image by [*Manfred Stege*](https://pixabay.com/users/manfredsteger-1848497/) via [*Pixabay*](https://pixabay.com/vectors/pixel-cells-pixel-network-3704044/).](img/network.png){width="400"}
...
3. **Historical/conceptual context:** "*The moderator-mediator variable distinction in social psychological research: conceptual, strategic, and statistical considerations*" by @baron1986. This classic paper is useful for terminology and historical context, but should be paired with the counterfactual causal-mediation sources below rather than used as the identification framework on its own.
...
5. **Recommended software bridge:** @tingley2014 provides the canonical peer-reviewed description of the `{mediation}` package and worked examples that connect mediator/outcome models to causal estimands and sensitivity analysis.
...
This week, we will not map scholarly materials by learning goal. Instead, the following are **recommended instructor resources** from which short student-facing excerpts can be selected; they are not intended as three cumulative required readings:
...
Following this introduction, teams will spend the rest of **week 10** working collaboratively on their assigned paper. This will involve mapping the study’s design, data, and analysis onto the course’s causal roadmap, identifying strengths and weaknesses, and critiquing methodological choices. Because published papers do not always name the roadmap stages explicitly, teams will also be asked to distinguish what the authors state directly from what must be reconstructed from the design, assumptions, estimand, statistical model, and analysis. Students will be expected to examine issues such as confounding, identifiability, and data-modelling assumptions. The emphasis will be on **deep engagement** with the chosen study rather than superficial summary.
...
In the last two weeks of this course (i.e., **weeks 12** and **13**), students will take the lead in executing a team-based project that encompasses every stage of the causal roadmap. This means the instructor will primarily support the students throughout the process, rather than delivering traditional lectures. This analysis will serve as a **final capstone project**, summarizing all the content covered in the course from weeks 1 to 9. The project aims to incorporate every stage of the roadmap introduced during the term, which consists of the following eight stages: (1) *research question*, (2) *causal model representing existing knowledge*, (3) *counterfactuals and causal parameters*, (4) *defining the observed-data and statistical problem*, (5) *model fitting (estimation)*, (6) *interpretation*, (7) *reporting*, and (8) *storytelling*. The student teams will remain the same as those formed during **weeks 10** and **11** for the **previous appraisal of a causal analysis**.
...
- Utilize an estimation procedure via `R` that is consistent with the causal model and the **implemented course pathway**. Propensity-score/IPW methods provide the standard core option; G-estimation or causal mediation analysis may be used when the corresponding optional/advanced module was taught and is appropriate to the research question.
...
- Compile the roadmap stages into a reproducible causal workflow, including code, visualizations, and documentation, to support transparency and reproducibility.
...
Starting with the second session of **week 12**, teams will conduct their causal analyses using the dataset in conjunction with the eight-stage causal roadmap. They will be expected to iteratively apply concepts such as graphical modelling with DAGs, SCMs, counterfactual definitions, identifiability assessment, and appropriate estimation methods, along with interpretation and storytelling. Note that we will use the **same dataset** for all teams, allowing instructors to evaluate different methodological approaches and assumptions reflected in each team’s application of the roadmap.
